...
| source-red band center `27°`, radius `< 38°`                                                      | alternate Destructive lightness anchor 적용 | 입력 red와 Destructive가 가까울 때 lightness preference를 바꾸는 `product-policy` predicate; 38°는 경험적으로 검증된 red 범위가 아니며 [고정-anchor 진단](../research/adversarial-audit.md#destructive-default-anchor-counterfactual)으로 별도 검토됨                                                                                                              |
