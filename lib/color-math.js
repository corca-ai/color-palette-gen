...
export function solveOklchContrast({
  ...
  if (!backgrounds.length || backgrounds.some((value) => !isHex(value))) {
    throw new TypeError("solveOklchContrast requires one or more hex backgrounds");
  }
  if (!(target >= 1 && Number.isFinite(target))) {
    throw new RangeError("solveOklchContrast target must be a finite ratio of at least 1");
  }
