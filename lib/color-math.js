...
/**
 * Describes the perceptual movement between two OKLCH colors. Delta E is the
 * Euclidean distance in the underlying Oklab space; the axis deltas remain
 * available so a designer can see which part of the color actually moved.
 */
export function oklchDifference(from, to) {
  const first = oklchToOklab(from);
  const second = oklchToOklab(to);
  const deltaL = to.l - from.l;
  const deltaC = to.c - from.c;
  const deltaH =
    from.c < 0.0001 || to.c < 0.0001 ? 0 : signedHueDelta(from.h, to.h);
  return {
    deltaL,
    deltaC,
    deltaH,
...
function contrastEvaluation(hex, backgrounds) {
  const ratios = backgrounds.map((background) => ({
    background,
    ratio: contrastRatio(hex, background),
  }));
  const limiting = ratios.reduce((lowest, current) =>
    current.ratio < lowest.ratio ? current : lowest,
  );
  return {
    ratio: limiting.ratio,
    ratios,
    limitingBackground: limiting.background,
  };
}
...
export function solveOklchContrast({
  ...
  const evaluate = (lightness) => {
    ...
    const contrast = contrastEvaluation(mapped.hex, backgrounds);
    return {
      ...mapped,
      ...contrast,
      passed: contrast.ratio >= target,
    };
  };
  ...
  const searchDarker = () => {
    ...
    if (!endpoint.passed) return { available: false, result: endpoint };
    ...
    return { available: true, result: evaluate(passing) };
  };
  const searchLighter = () => {
  ...
  const searched = [
    { direction: "darker", ...searchDarker() },
    { direction: "lighter", ...searchLighter() },
  ];
  const eligible = searched.filter(
    (solution) =>
      solution.available &&
      (direction === "nearest" || direction === solution.direction),
  );
  const selected = eligible.sort(
    (first, second) =>
      Math.abs(first.result.color.l - startLightness) -
      Math.abs(second.result.color.l - startLightness),
  )[0]?.result;
  const solutions = searched.map(
    ({ direction: solutionDirection, available, result }) => ({
      direction: solutionDirection,
      available,
      eligible: direction === "nearest" || direction === solutionDirection,
      selected: available && result.hex === selected?.hex,
      hex: result.hex,
      color: result.color,
      ratio: result.ratio,
      ratios: result.ratios,
      limitingBackground: result.limitingBackground,
      lightnessDelta: result.color.l - color.l,
    }),
  );
  ...
  const fallbacks = ["#000000", "#FFFFFF"]
    .map((hex) => ({ hex, ...contrastEvaluation(hex, backgrounds) }))
  ...
  return {
    ...
    limitingBackground: fallback.limitingBackground,
