      ...
      ...(mapped.solutions
        ? {
            contrast: {
              ...
              limitingBackground: mapped.limitingBackground,
              ratios: mapped.ratios,
...
function createOnColorToken(name, background, source, traces) {
  const scores = ["#FFFFFF", "#000000"]
    .map((hex) => ({ hex, ratio: contrastRatio(hex, background) }))
    .sort((a, b) => b.ratio - a.ratio);
  const selected = scores[0];
  const trace = createTrace(name, source);
  const selectedOklch = rgbToOklch(hexToRgb(selected.hex));
  attachColorArtifact(trace, selectedOklch, oklchToHex(selectedOklch));
  scores.forEach((score) =>
    addStep(
      trace,
      "contrast",
      `${score.hex} on ${background}: ${score.ratio.toFixed(2)}:1.`,
      background,
      `${score.hex} · ${score.ratio.toFixed(2)}:1`,
    ),
  );
  addStep(
    trace,
    "final",
    "Selected the black or white foreground with stronger contrast on the filled accent.",
    background,
    selected.hex,
  );
  if (selected.ratio < 4.5) {
    trace.warnings.push(
      `CONTRAST_UNRESOLVED: ${name} reached ${selected.ratio.toFixed(2)}:1 of the 4.5:1 target.`,
    );
  }
  traces[name] = trace;
  return [selected.hex, name];
}
...
function addAccentFamily(
  ...
  tokens.push(
    createOnColorToken(
      `${prefix} accent on-color`,
      accent[0],
      `${accentName} + filled-content contrast contract`,
      traces,
    ),
  );
}
