...
export function completeHarmonyColor(primary, secondaryHex, params) {
  ...
  if (harmony.includes("monochromatic")) {
    ...
    const occupied = hueDistance(secondary.h, arms[0].hue) <= hueDistance(secondary.h, arms[1].hue) ? 0 : 1;
    ...
    chromaMultiplier = 0.62;
    relation = "Reused secondary hue; the third role varies lightness and chroma.";
    edgeLabel = "reuse H · vary L/C";
  } else {
    const hues = params.hueOffsets.map((offset) => (primary.h + offset + 360) % 360);
    targetHue = hueDistance(secondary.h, hues[0]) > hueDistance(secondary.h, hues[1]) ? hues[0] : hues[1];
    relation = "Selected the configured position furthest from user secondary.";
    edgeLabel = `complete ${signedHueDistance(primary.h, targetHue).toFixed(0)}°`;
  }

  const mapped = mappedHarmonyColor(primary, params, targetHue, chromaMultiplier);
  return {
    hex: mapped.hex,
